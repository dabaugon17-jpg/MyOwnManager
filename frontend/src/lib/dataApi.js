...
export const getDashboard = async ({ filter = "month", vendedor_id = null } = {}) => {
  ...
  const [{ data: products, error }, group] = await Promise.all([
  ...
  const inventory = (products || []).filter((p) => p.estado === "inventario");

  const facturacion_total = sold.reduce((s, p) => s + (p.precio_venta || 0), 0);
  const inversion = (products || []).reduce((s, p) => s + (p.precio_compra || 0), 0);
